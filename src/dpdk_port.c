...
static int create_pool(struct if_state *handle){
    ...
    handle->mbuf_pool = rte_pktmbuf_pool_create(name,
                                                ...
                                                rte_socket_id());
    if(!handle->mbuf_pool) {DBG( "mempool create failed\n"); return -1;}
    return 0;
}
...
void port_print_info(uint16_t port_id, const char* tag){
    ...
    DBG("[%s] port %u: MAC %02x:%02x:%02x:%02x:%02x:%02x, link %s %u Mbps\n",
        tag, port_id,
                    ...
                    uint32_t mbufs, uint32_t cache)
{
    ...
    if (n < 2) { DBG( "need >= 2 DPDK ports, have %u\n", n); return -1; }
